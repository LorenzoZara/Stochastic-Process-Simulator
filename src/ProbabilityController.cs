...
namespace StochasticProcessSimulator
{
    public class ProbabilityController
    {
        ...
        //returns the outcome of a Bernoulli distribution B(1, p)
        public int bernoulli_outcome(double p)
        {
        ...
        //returns the outcome of a binomial distribution B(n, p)
        private int binomial_outcome(int n, double p)
        {
            List<int> outcome = createListOfBernoullis(n, p);

            return outcome.Sum();
        }

        //returns the outcome of a Rademacher distribution
        public int rademacher_outcome()
        {
            double p = 0.5;
        ...
        //return the outcome of a Poisson distribution (approximation)
        public int poisson_outcome(int k, double rate)
        {
            return binomial_outcome(k, rate / (double)k);
        }

        public double lognormal_outcome(double mean, double dev)
        {
            return Math.Exp(normal_outcome(mean, dev));
        }

        // MARSAGLIA POLAR METHOD: https://en.wikipedia.org/wiki/Marsaglia_polar_method
        private static double spare;
        private static bool hasSpare = false;
        public double normal_outcome(double mean, double stdDev) // Marsaglia polar method 
        {
            if (hasSpare)
            {
                hasSpare = false;
